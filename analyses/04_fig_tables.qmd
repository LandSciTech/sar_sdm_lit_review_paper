...
This code creates figures and tables used in the manuscript and supplementary material.
...
# remove unpublished Stolar paper
sarsdm <- sarsdm %>% filter(!str_detect(doc_citation, "Stolar"))

# load paper database
paper_db <- read.csv(paste0(path,"/data/SAR-SDM_paper_database_clean.csv"))

# remove unpublished Stolar paper
paper_db <- paper_db %>% filter(!str_detect(doc_citation, "Stolar"))
...
```{r, echo = FALSE}
#| fig-cap: "A) Cumulative number of peer-reviewed papers that include an SDM of Canadian species at risk across time, based on findings from the literature review. SDM-CC papers (green) are a subset of the SDM papers (purple) that include climate change projections."
#| fig-height: 5
#| fig-width: 5

trend_dat <- bind_rows(sdm_temp_trends, cc_sdm_temp_trends) %>%  
  mutate(label = factor(sdm_type,
                        levels = c("sdm_na","cc_sdm_na"),
                        labels = c("SDM", "SDM-CC")))
...
## Trends in available SDM literature
...
fig_ppr_db <- paper_db %>%
  ...
  mutate(label = paste0(sdm_na, sdm_ca, cc_sdm_na, cc_sdm_ca),
         label = factor(label,
                        ...
                        labels = c("remove", "CC-SDM", "CC-SDM", "SDM", "SDM"))
         ) %>%
  filter (label != "remove") 
...
fig_pnl_num_ppr <- fig_ppr_db %>%
  mutate(taxonomic_group = factor(taxonomic_group,
         ...
         label = factor(label, levels = c("CC-SDM", "SDM"), 
                        labels = c("SDM-CC", "SDM-No CC")),
         label = str_wrap(label, width = 15)
) %>%
  ggplot(aes(x = fct_rev(taxonomic_group), y = n_ppr, fill = fct_rev(label))) +
...
fig_pnl_prop_sp_model <- na_db_alt %>%
  ...
  scale_fill_manual(values = c("#b8b8b8", # no SDM
                               "#ea801c", # SDM-No CC
                               "#1a80bb", #SDM-CC
                               "#ae017e" # SDM-CC &\nSDM-No CC                              
                               )) + 
...
```{r, echo = FALSE}
dat_n_sdm <- sarsdm %>%
  select(species, taxonomic_group, sara_status_adj, n_sdm, n_sdm_ca, n_cc_sdm, 
         n_cc_sdm_ca, range_size_America) %>%
  # arrange(-(n_sdm)) %>%
  arrange(-(n_sdm)) %>%
  unique() %>%
  mutate(range_size_America = replace_na(range_size_America, 0))

dat_med_n_sdm <- dat_n_sdm %>%
  summarise(
    med_n_sdm = median(n_sdm), IQR_n_sdm = IQR(n_sdm),
    q1 = quantile(n_sdm, probs = 0.25),
    q2 = quantile(n_sdm, probs = 0.75),
    .by = "taxonomic_group"
  ) %>%
  arrange(-med_n_sdm)

lab_n_sdm <- dat_n_sdm %>% filter(n_sdm >= 9)

jttr <- position_jitter(width = 0.2, height = 0, seed = 1234)

plt_n_sdm <- dat_n_sdm %>%
  ggplot(aes(n_sdm, range_size_America, col = taxonomic_group)) +
  geom_point(position = jttr) +
  scale_x_continuous(breaks = 0:16, expand = expansion(add = c(0.5, 5))) +
  scale_y_sqrt(
    breaks = c(0, 10^(5:7), 2 * 10^7),
    expand = expansion(mult = c(0.025, 0.05))
  ) +
  scale_color_brewer(palette = "Paired") +
  geom_text(
    data = lab_n_sdm,
    aes(label = species, x = n_sdm + 0.25),
    col = "black",
    position = jttr,
    show.legend = FALSE,
    hjust = 0
  ...
  theme_classic() +
  labs(
    x = "Number of papers", y = "Range size (square root transformed)",
    col = "Taxonomic group"
  ) +
  coord_cartesian(clip = "off")


jpeg(file.path(path,"/figures/fig_n_sdm_papers.jpeg"), units="in", width=7.5, height=4, res=1000) 
plt_n_sdm 
dev.off()

pdf(file.path(path,"/figures/fig_n_sdm_papers.pdf"), width=7.086, height=4) 
plt_n_sdm 
dev.off()

# Number of species only included in Stolar et al by taxonomic group
paper_db %>%
  filter(sdm_na > 0) %>%
  distinct(taxonomic_group, species, doc_citation, sdm_na) %>%
  mutate(is_stolar = str_detect(doc_citation, "Stolar")) %>%
  mutate(sdm_na_not_stolar = ifelse(is_stolar, 0, sdm_na)) %>%
  group_by(taxonomic_group, species) %>%
  summarise(n_sdm = sum(sdm_na), n_sdm_wo_stolar = sum(sdm_na_not_stolar)) %>%
  group_by(taxonomic_group) %>%
  summarise(n_sp_w_sdm = sum(n_sdm > 0), n_sp_w_sdm_wo_stolar = sum(n_sdm_wo_stolar > 0)) %>%
  mutate(change_wo_stolar = n_sp_w_sdm - n_sp_w_sdm_wo_stolar) %>%
  select(taxonomic_group, n_sp_w_sdm, change_wo_stolar) %>%
  mutate(taxonomic_group = str_remove(taxonomic_group, "\\(terrestrial\\)")) %>%
  kable()
...
fig_pnl_sdm_re <- re_na_sdm_bin %>%
  ...
  theme_classic() +
  scale_fill_manual(values=rev(c("grey", "#feebe2", "#fbb4b9", "#f768a1", "#ae017e"))) +
  ylab("Proportion of species") +
  xlab(element_blank()) +
...
fig_pnl_cc_sdm_re <- re_na_cc_sdm_bin %>%
  ...
  geom_text(aes(taxonomic_group, 1.05, label = n_taxa, fill = NULL), size = 3, colour = '#36454F', data = sp_taxa_label_re) +
  theme_classic() +
  scale_fill_manual(values = c("#d7301f", "#fc8d59", "#fdcc8a", "grey")) +
  ylab("Proportion of species") +
  xlab(element_blank()) +
  labs(fill = "Number of papers") +
  theme(axis.text.x = element_text(size = 10),
...
pdf(paste0(path,"/figures/fig_research_effort.pdf"), width=6, height=4)
plot_grid(fig_pnl_sdm_re, fig_pnl_cc_sdm_re,
          nrow = 2,
          labels = "AUTO")
dev.off()
...
m1_plt <- m1_plt_data %>% 
  ...
  geom_point(shape = m1_plt_data$group, size = 3, stroke = 1.5)+ 
...
m2_plt_data <- sjPlot::plot_model(m2_std_clean, sort.est = FALSE, title = "CC-SDM inclusion",
                               transform = NULL)$data  %>% 
  mutate(group = case_when(str_detect(term, "Amphibians") ~ 1,
                           str_detect(term, "Birds") ~ 1,
                           str_detect(term, "Lichens") ~ 1,
...
m2_plt <- m2_plt_data %>% 
  ggplot(aes(term, estimate))+
  theme_classic()+
  ggtitle("SDM-CC inclusion")+
  geom_hline(yintercept = 0, col = "grey", size = 0.2)+
  geom_segment(aes(y = estimate + 0.038, yend = conf.high, xend = term))+
  geom_segment(aes(y = conf.low, yend = estimate - 0.038, xend = term))+
  geom_point(shape = m2_plt_data$group, size = 3, stroke = 1.5)+ 
  scale_color_manual(values = c("black", "black"))+
  ylim(-0.8, 3.3)+
  coord_flip()+
  #scale_x_discrete(labels = labs1)+
...
m7_plt_data <- sjPlot::plot_model(m7_std_clean, sort.est = FALSE, title = "SDM research effort",
                               transform = NULL)$data  %>% 
  mutate(group = case_when(str_detect(term, "Amphibians") ~ 1,
                           str_detect(term, "Birds") ~ 1,
                           str_detect(term, "Lichens") ~ 1,
...
m7_plt <- m7_plt_data %>% 
  ggplot(aes(term, estimate))+
  theme_classic()+
  ggtitle("SDM research effort")+
  geom_hline(yintercept = 0, col = "grey", size = 0.2)+
  geom_segment(aes(y = estimate + 0.038, yend = conf.high, xend = term))+
  geom_segment(aes(y = conf.low, yend = estimate - 0.038, xend = term))+
  geom_point(shape = m7_plt_data$group, size = 3, stroke = 1.5)+ 
  scale_color_manual(values = c("black", "black"))+
  ylim(-1.05, 1.15)+
  coord_flip()+
  #scale_x_discrete(labels = labs1)+
...
m8_plt_data <- sjPlot::plot_model(m8_std_clean, sort.est = FALSE, title = "CC-SDM research effort",
                               transform = NULL)$data  %>% 
  mutate(group = case_when(str_detect(term, "Amphibians") ~ 1,
                           str_detect(term, "Birds") ~ 1,
                           str_detect(term, "Lichens") ~ 1,
...
m8_plt <- m8_plt_data %>% 
  ggplot(aes(term, estimate))+
  theme_classic()+
  ggtitle("SDM-CC research effort")+
  geom_hline(yintercept = 0, col = "grey", size = 0.2)+
  geom_segment(aes(y = estimate + 0.038, yend = conf.high, xend = term))+
  geom_segment(aes(y = conf.low, yend = estimate - 0.038, xend = term))+
  geom_point(shape = m8_plt_data$group, size = 3, stroke = 1.5)+ 
  scale_color_manual(values = c("black", "black"))+
  ylim(-1.05, 1.15)+
  coord_flip()+
  #scale_x_discrete(labels = labs1)+
...
# Set limits the same across plots
# bind_rows(m8_plt_data, m7_plt_data, m1_plt_data, m2_plt_data) %>%
#   summarise(min = min(conf.low), max = max(conf.high))
#          min      max
# 1 -0.3919535 1.582384


cowplot::plot_grid(m1_plt, m7_plt, m2_plt, m8_plt, nrow = 2, labels = c('A', 'B', 'C', 'D'))
```

```{r}
#save figure
jpeg(paste0(path,"/figures/fig_glm_results.jpeg"), units="in", width=7.5, height=6, res=1000)
cowplot::plot_grid(m1_plt, m7_plt, m2_plt, m8_plt, nrow = 2, labels = c('A', 'B', 'C', 'D'))
dev.off()

pdf(paste0(path,"/figures/fig_glm_results.pdf"), width=7.086, height=6)
cowplot::plot_grid(m1_plt, m7_plt, m2_plt, m8_plt, nrow = 2, labels = c('A', 'B', 'C', 'D'))
dev.off()
...
## Use and applications of SDMs in listing documents
...
fig_pnl_prop_doc_sdms_ld <- tbl_taxa_doc_sdm_comb_ld %>%
  ...
  xlab(element_blank()) +
  scale_fill_manual(values = c("#b8b8b8", # no SDMs
                               "#ea801c", # SDM w/out CC
                               "#1a80bb", #CC-SDM
                               '#ae017e' # SDM-CC &\nSDM-No CC
                               )) + 
...
fig_pnl_sdm_uses_ld <- bind_rows(cc_sdm_sections, oth_sdm_sections) %>%
  ...
  scale_fill_manual(values = rev(c("#1a80bb", #SDM-CC
                               "#ea801c" # SDM-No CC
                               )), drop = TRUE) +
...
fig_ldtype_doc_comb_sdm <- fig_ldtype_doc_sdm_comb_ld %>%
  mutate(label = factor(sdm_type,
                        levels = c("No SDMs", "SDM w/out CC", "CC-SDM", 
                                   "CC-SDM & SDM w/out CC"),       labels = c("No SDM", "SDM-No CC", "SDM-CC", "SDM-CC")),
...
pdf(paste0(path,"/figures/fig_ld_stats.pdf"), width=5, height=8)
fig_ld_stats
dev.off()
```
...
## Availability vs use of SDM-CC papers
...
```{r}
# Multi species papers more common for Amphibians?
dat_n_sp_paper <- paper_db %>% filter(sdm_na == 1) %>% 
  group_by(paperID, doc_citation, taxonomic_group) %>% 
  summarise(n_sp = n_distinct(speciesID), .groups = "drop") %>%
  group_by(paperID) %>% 
  mutate(tot_n_sp = sum(n_sp)) 

dat_n_sp_paper %>% 
  group_by(taxonomic_group) %>% 
  summarise(mean_n_sp = mean(n_sp), n_1 = sum(n_sp == 1), n_gr_1 = sum(n_sp > 1)) %>% 
  arrange(desc(mean_n_sp)) %>% 
  mutate(pct_gr_1 = n_gr_1/(n_1 + n_gr_1))
...
supp_fig_pnl_sdm_uses_ld_by_doctype <- bind_rows(cc_sdm_sections, oth_sdm_sections) %>%
  ...
  #geom_text(aes(label = n_doc))+
  # colours if using the application types as colours
  scale_fill_manual(values = c("#1a80bb", #SDM-CC
                               "#ea801c" # SDM-No CC
                               ), drop = TRUE) +
  ylab("Number of documents") +
  xlab(element_blank()) +
...
supp_fig_diff_re_cc_sdm <- re_diff_cc_sdm_m %>%
  ...
  ggtitle("SDM-CC (subset of SDMs)")+
  geom_bar(position = "stack", stat = "identity") +
  geom_text(aes(taxonomic_group, 1.05, label = n_sp_taxa, fill = NULL), 
          size = 3, colour = '#36454F', data = re_diff_taxa_label) +
  theme_classic() +
...
supp_fig_sdm_re_ca <- re_ca_sdm_bin %>%
  ...
  geom_text(aes(taxonomic_group, 1.05, label = n_taxa, fill = NULL), size = 3, colour = '#36454F', data = sp_taxa_label_re) +
  theme_classic() +
  scale_fill_manual(values=c('#2f006a', '#553b92','#8d85be','#d0d1e5',"#b8b8b8")) +
  ylab("Proportion of species") +
  xlab(element_blank()) +
  labs(fill = "Number of papers") +
  theme(axis.text.x = element_text(size = 10),
...
supp_fig_cc_sdm_re_ca <- re_ca_cc_sdm_bin %>%
  ...
  geom_text(aes(taxonomic_group, 1.05, label = n_taxa, fill = NULL), size = 3, colour = '#36454F', data = sp_taxa_label_re) +
  theme_classic() +
  scale_fill_manual(values = c('#0c5c21','#379e4b','#dff4da',"#b8b8b8")) +
  ylab("Proportion of species") +
  xlab(element_blank()) +
  labs(fill = "Number of papers") +
  theme(axis.text.x = element_text(size = 10),
