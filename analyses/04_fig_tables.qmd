...
dat_n_sdm <- sarsdm %>%
  select(species, taxonomic_group, sara_status_adj, n_sdm, n_sdm_ca, n_cc_sdm, n_cc_sdm_ca, range_size_America) %>% 
  #arrange(-(n_sdm)) %>%
  arrange(-(n_sdm))%>%
  unique() %>% 
...
plt_n_sdm <- dat_n_sdm %>% 
  ...
  scale_y_sqrt(breaks = c(0, 10^(5:7), 2*10^7), 
  ...
  theme_classic()+
  labs(x = "Number of papers", y = "Range size (square root transformed)", col = "Taxonmonic group")
