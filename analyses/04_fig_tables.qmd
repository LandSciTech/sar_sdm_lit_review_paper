...
fig_pnl_cum_temp_trends_sdm_lit <- trend_dat %>% 
  ...
  theme_classic() +
  scale_color_manual(values = c('#562988', 
                                '#33A02B')) + 
...
geom_col()+
scale_fill_manual(name = NULL, values = rev(c('grey', # no SDM
                               '#1c63a5', # SDM-No CC
                               '#2c9320', #SDM-CC
                               '#562888' # SDM-CC &\nSDM-No CC                        
                               )),
...
fig_pnl_sdm_re <- re_na_sdm_bin %>%
  ...
  theme_classic() +
  scale_fill_manual(values=c("#4A1486", "#6A51A3", "#807DBA",
                             "#9E9AC8", "grey")) +
  ylab("Proportion of species") +
  xlab(element_blank()) +
...
fig_pnl_cc_sdm_re <- re_na_cc_sdm_bin %>%
  ...
  geom_text(aes(taxonomic_group, 1.05, label = n_taxa, fill = NULL), size = 3, colour = '#36454F', data = sp_taxa_label_re) +
  theme_classic() +
  scale_fill_manual(values = c("#238B45", "#74C476", "#BAE4B3", "grey")) +
  ylab("Proportion of species") +
  xlab(element_blank()) +
  labs(fill = "Number of papers") +
  theme(axis.text.x = element_text(size = 10),
