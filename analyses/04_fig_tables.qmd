...
```{r, echo = FALSE}
#clean environment
rm(fig_pnl_prop_doc_sdms_ld, fig_pnl_temp_trends_ld,
   ld_temp_trends, ld_sdm_temp_trends, ld_oth_sdm_temp_trends, ld_cc_sdm_temp_trends
   )
```
...
```{r}
# save figure 4V2
jpeg(paste0(path,"/figures/fig_ld_sdm_applications_V2.jpeg"), units="in", width=5, height=5, res=1000)
plot_grid(get_plot_component(fig_ldtype_doc_comb_sdm, "guide-box", 
                             return_all = TRUE)[[3]],
          fig_ldtype_doc_comb_sdm+guides(fill = "none"), fig_pnl_sdm_uses_ld, 
          rel_heights = c(0.05, 0.35, 0.6),
          nrow = 3, labels = c("", "A", "B"))
dev.off()
```
...
rm(fig_pnl_sdm_uses_ld,
   cc_sdm_sections, oth_sdm_sections)
```

**Availability vs use analysis**
...
## 3.3 Trends in available SDM literature
...
### 3.3.1 Inclusion and research effort models
...
```{r}
# # save supp figure 1
# jpeg(paste0(path,"/figures/supp_ld_sdm_presence_by_doctype.jpeg"), units="in", width=6, height=4, res=1000)
# supp_fig_ldtype_doc_comb_sdm
# dev.off()
```
...
To get information for caption for Table S3, we need to know which species are only included in Canada-exclusive models.
...
**Table S4-S7** are results from each of the North American GLMs discussed in the main text of the paper, and shown in the Figure in the main tedxt. They are written and saved as part of the statistical analysis code, and can be found at the following file path:

-   Table S4: sarsdm/outputs/model_results/model1.csv
-   Table S5: sarsdm/outputs/model_results/model2.csv
-   Table S6: sarsdm/outputs/model_results/model7.csv
-   Table S7: sarsdm/outputs/model_results/model8.csv

**Tables S8-S11** are results from each of the Canada-inclusive GLMs, only shown in the Supplementary. They are written and saved as part of the statistical analysis code, and can be found at the the following file path:

-   Table S8: sarsdm/outputs/model_results/model3.csv
-   Table S9: sarsdm/outputs/model_results/model4.csv
-   Table S10: sarsdm/outputs/model_results/model5.csv
-   Table S11: sarsdm/outputs/model_results/model6.csv
